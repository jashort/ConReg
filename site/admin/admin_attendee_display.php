...
if (isset($_GET['id'])) {
    ...
    if (hasRight("attendee_edit_notes")) {
        attendeeUpdateNotes($_POST['id'], $_POST['notes']);
        logMessage($_SESSION['username'], 60, "Updates notes for attendee ID " . $_POST['id']);
        redirect("/index.php");
    } else {
        die("You do not have the correct access level to edit notes.");
    }
}
?>
<!DOCTYPE html>
...
<div class="container">
    ...
    <div class="jumbotron">
        ...
        <fieldset id="personalinfo">
            <label>Name: </label>
            <span class="display_text"><?php echo $attendee->first_name; ?> <?php echo $attendee->last_name; ?></span>
            <br />
    ...
    <span class="display_text"><?php echo $attendee->checked_in; ?>
        ...
        <fieldset id="notes">
            <label>Notes:</label>
            <?php if (hasRight("attendee_edit_notes")) { ?>
                <form action="admin_attendee_display.php" method="post">
                    <input type="hidden" name="id" value="<?php echo $attendee->id; ?>" />
                    <textarea name="notes" rows="5" cols="80"><?php echo $attendee->notes; ?></textarea><br />
